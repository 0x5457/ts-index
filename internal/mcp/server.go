...
func newSemanticSearchTool() mcp.Tool {
    return mcp.NewTool(
        ...
        mcp.WithString("project", mcp.Description("Optional project path to index into memory before searching")),
    )
}
...
func handleLSPCompletion(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
    project, err := req.RequireString("project")
    if err != nil {
        return mcp.NewToolResultError(err.Error()), nil
    }
